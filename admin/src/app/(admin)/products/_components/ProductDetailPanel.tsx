...
type Tab = "overview" | "assets" | "pricing" | "prices";
...
export default function ProductDetailPanel({ id, onProductChanged }: Props) {
  ...
  const tabs: { id: Tab; label: string; count?: number }[] = [
    ...
    { id: "pricing", label: "Pricing", count: scenarios.length },
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            >
              <Package className="w-4 h-4" style={{ color: "var(--mc-cream-subtle)" }} />
            </div>
            <div>
              ...
              <p className="text-[11px] mt-0.5" style={{ color: "var(--mc-text-muted)" }}>
                {product.brand || "No brand"} — {CATEGORY_LABELS[product.category] ?? product.category}
              </p>
            </div>
          </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderBottom: isActive ? "2px solid var(--mc-cream)" : "2px solid transparent",
                  fontWeight: isActive ? 600 : 400,
                }}
                onMouseEnter={(e) => {
                  if (!isActive) e.currentTarget.style.color = "var(--mc-text-secondary)";
                }}
                onMouseLeave={(e) => {
                  if (!isActive) e.currentTarget.style.color = "var(--mc-text-muted)";
                }}
              >
                {t.label}
                {t.count !== undefined && (
                  <span className="ml-1.5" style={{ color: "var(--mc-text-muted)" }}>
                    {t.count}
                  </span>
                )}
              </button>
